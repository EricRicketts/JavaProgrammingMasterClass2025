...
Records are good for simple data because they automatically provide a constructor, accessor methods, toString(), equals(), and hashCode(). Their fields are final, so once a record is created, its state cannot be changed. This makes records useful for small, simple data objects whose main purpose is to store values.## 2. When would a record be a bad choice?
Your answer:
...
Records provide **accessor methods**, not setter methods. For example, if you have:``` java
public record StudentInfo(String name, int id, double gpa) {
}
```

Java gives you methods like:``` java
student.name();
student.id();
student.gpa();
```

It does not give you:``` java
...
Automatically gives you something conceptually like:``` java
...
It does not automatically give you:``` java
public Coordinate()

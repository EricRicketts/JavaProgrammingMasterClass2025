...
1. one full constructor
2. one partial constructor
3. constructor chaining

### In addition provide an instructor style marksheet for the following:

1. Correctness - does the code meet the specification?
2. Encapsulation
3. Immutability - where applicable
4. Validation - is it well separated?
5. Readability - fields, methods, method names, tests, test names
6. Testing
7. Design
8. Overall Grade
...
```java
ErrorMessages.NULL_VALUE_MESSAGE_FOR_NAME.getErroMessage()
```
...
But a entity such as `Person` can reasonably change over time.
...
Right now the production code uses constants, but the tests still compare against raw strings. That is not wrong, but if the expected message changes, you would need to update both the enum and the tests. For example:
``` java
   assertEquals(
...
In the two-argument constructor section, the empty-name and blank-name tests are still calling:
``` java
...
Since those tests are inside the two-argument constructor test group, they should probably be:
``` java
   new Person("", 43)
```
...
All two-argument constructor tests actually call the two-argument constructor
``` java
   new Person("", 43)
   new Person(" ", 43)
   new Person(null, 12)
...
All expected exception messages now come from ErrorMessages
``` java
   ErrorMessages.NULL_VALUE_MESSAGE_FOR_NAME.getErrorMessage()
```
...
you have unsaved changes, the IDE/project view is out of sync, you want me to focus only on a specific version, or you want to make sure I’m reviewing exactly what you’re seeing.
...
2. setter validation
3. null, empty, and blank string checks
4. age validation
5. constants for error messages
6. tests for both constructors
7. tests for getters and setters
8. tests for exception types and messages
9. less brittle toString() tests
...
1. name is final
2. age and city are mutable through setters
3. constructors validate before assignment
4. setters reuse the same validation logic
5. helper methods return validated values
6. error messages come from ErrorMessages

That is a clean pattern.
The constructor is especially readable:
``` java
this.name = validateNameAndReturn(name);
this.age = validateAgeAndReturn(age);
this.city = validateCityAndReturn(city);
...
1. three-argument constructor invalid cases
2. two-argument constructor invalid cases
3. getter behavior for both constructors
4. setter rejection cases
5. successful setter updates
6. toString() content checks

That is more than enough coverage for a small beginner/intermediate OOP class.  Are there still possible improvements?

Yes, but they are mostly optional polish, not necessary fixes.

1. You could reduce repeated test code

There is some repetition in the tests, especially around this pattern:
```java
assertEquals(
...
1. You could consider parameterized tests later

For example, empty and blank values are similar cases:
``` java
""
" "
```
...
Your current `toString()` test is intentionally not tied too tightly to the
exact format, which is good. The only thing to watch is that splitting
strings with regex can become a little harder to read than a few direct
contains assertions.

For example, this style is simpler:
``` java
...
1. You could decide whether name should stay immutable

Right now, name is final, while age and city can change. That makes sense:
a person’s name is fixed in this model age and city can change.  So I would leave that as-is unless you want to model name changes too.
...
7. less brittle toString() testing
...
Something like this is very readable:
``` java
String result = firstPerson.toString();
...
3. constructor chaining
4. exception testing
5. mutable vs immutable fields
6. organized nested tests
7. better toString() testing
...
the questions, your answers, and whether you want a gentle review or a strict instructor-style critique.
...
3. missing details
4. misconceptions
5. and whether your wording shows real understanding.
...
The foundation you’re building.  You are already practicing several skills that matter a lot:
...
A useful warning, though as you keep learning, you’ll eventually run into developers who say things like:
...
Sometimes they’ll be right about context. But sometimes they’ll be skipping the learning step.  At your stage, “overthinking” small examples is actually useful, because it builds judgment. Later, you’ll know when to simplify because you’ll understand what you are simplifying away.
...
Math + programming is a great retirement combination.  Using Art of Problem Solving material is an excellent idea if you enjoy deep problem solving. AoPS-style math is less about memorizing procedures and more about learning how to think through problems creatively. That pairs very naturally with programming.
...
Project Euler especially combines math and programming in a very satisfying way.  A good long-term learning rhythm, if programming and math are going to be lifelong hobbies, I would suggest a rhythm like this:
...
One especially good future project idea, when you get further along, you could build your own:
...
Whether or not you ever program professionally, you’re already doing something valuable: you’re becoming a better thinker.  That is one of the best reasons to program.
...
2. Write your solution explanation, even a rough explanation is fine.
3. Bring it here

You could say:
Here is the problem, here is my solution. Please critique my reasoning but don’t just give me the answer.  That would let me act more like a math coach than an answer machine.
...
And the best part, you can build it slowly as your Java skill grows. Each little math idea can become a small Java exercise:

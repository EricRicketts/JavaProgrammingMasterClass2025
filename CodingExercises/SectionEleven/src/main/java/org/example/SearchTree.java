...
public class SearchTree implements NodeList {
    ...
    public String addItem(ListItem newItem) {
        ...
        while (currentItem != null) {
            ...
            if (comparison < 0) {
                ...
                // equal, so don't add
                return newItem.getValue() + " is already present";
            }
        }
    ...
    private void performRemoval(ListItem item, ListItem parent) {
        ...
        if (item.next() == null) {
            ...
            // neither left nor right are null, deletion is a lot trickier!
            ...
            if (leftmostParent == item) {
                ...
                // node (the one that now contains the duplicate value)
    ...
    private void traverseHelper(ListItem root, StringBuilder sb) {
        // recursive method
        if (root != null) {
            traverseHelper(root.previous(), sb);
            sb.append(root.getValue()).append("\n");
            traverseHelper(root.next(), sb);
        }
    }
}

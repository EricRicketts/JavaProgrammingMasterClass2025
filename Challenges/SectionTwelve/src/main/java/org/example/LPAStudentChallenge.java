...
public class LPAStudentChallenge extends StudentChallenge
    implements QueryItemChallenge {

    private BigDecimal percentComplete;

    public LPAStudentChallenge() {
    ...
    public int compareTo(StudentChallenge other) {
        int initialCompare = super.compareTo(other);
        if (initialCompare != 0) return initialCompare;
        LPAStudentChallenge lpaStudent = (LPAStudentChallenge) other;
        return this.getPercentComplete().compareTo(lpaStudent.getPercentComplete());
    }
    @Override
